...
def grab_intraday(symbol):
    data = None
    r = requests.get('https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&interval=15min&symbol=' + symbol + '&apikey=' + API_key)
    if r.status_code == 200:
        print('Successfully Connected')
        data = r.json()
        # print(data)
        file = symbol + 'intra.csv'
        file = Stock_Prices / file
        fd = open(file, 'w')
